...
export function renderComposition(job, imageFileName) {
...
<head>
  ...
  <style>
    *{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#061a38;font-family:Arial,Helvetica,sans-serif}
    ...
    .brand{position:absolute;z-index:2;top:${vertical ? 88 : 55}px;left:${vertical ? 76 : 68}px;font-size:${vertical ? 34 : 25}px;font-weight:900;letter-spacing:.19em}
    ...
    .media{position:absolute;z-index:2;left:${vertical ? 76 : 68}px;right:${vertical ? 76 : 520}px;top:${vertical ? 290 : 215}px;height:${vertical ? 820 : 620}px;border-radius:48px;background:#fff;display:flex;align-items:center;justify-content:center;padding:58px;box-shadow:0 36px 80px rgba(0,0,0,.28);overflow:hidden}
    ...
    .copy{position:absolute;z-index:2;left:${vertical ? 76 : 610}px;right:${vertical ? 76 : 68}px;top:${vertical ? 1190 : 185}px;bottom:${vertical ? 110 : 90}px;display:flex;flex-direction:column;justify-content:center}
    h1{margin:0;font-size:${vertical ? 84 : 62}px;line-height:.98;letter-spacing:-.04em;font-weight:900}p{margin:30px 0 0;color:rgba(255,255,255,.72);font-size:${vertical ? 34 : 25}px;line-height:1.35;font-weight:600}
...
<body>
  <main id="stage" data-composition-id="briland-product" data-no-timeline data-start="0" data-duration="${duration}" data-width="${width}" data-height="${height}">
    <div id="briland-brand" class="brand clip" data-start="0" data-duration="${duration}" data-track-index="1">BRILAND</div>
