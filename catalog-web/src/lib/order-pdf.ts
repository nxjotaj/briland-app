...
export async function buildOrderPdf(order: SalesOrder) {
  ...
  for(const item of items){
    ...
    page.drawText(`${item.effectiveDiscountPercent}%`,{x:418,y:y-10,size:7.5,font:regular,color:ink}); page.drawText(money(item.unitPrice),{x:458,y:y-10,size:7.5,font:regular,color:ink}); page.drawText(money(item.lineTotal),{x:514,y:y-10,size:7.5,font:bold,color:ink}); y-=rowHeight;
  }
...
export async function stockPdfFile(rows: SalesStock[]) {
  ...
  const head=()=>{page.drawRectangle({x:0,y:PAGE.height-52,width:PAGE.width,height:52,color:navy});page.drawText("BRILAND  |  POSICAO DE SALDO",{x:35,y:PAGE.height-33,size:16,font:bold,color:rgb(1,1,1)});page.drawText(`Pagina ${pageNumber}`,{x:510,y:PAGE.height-31,size:7,font:regular,color:rgb(.8,.84,.9)});y=PAGE.height-75;page.drawRectangle({x:35,y:y-20,width:525,height:23,color:yellow});[["CODIGO",42],["PRODUTO",125],["FISICO",420],["RESERVADO",468],["DISPONIVEL",520]].forEach(([v,x])=>page.drawText(String(v),{x:Number(x),y:y-13,size:7,font:bold,color:navy}));y-=27;};head();
  rows.forEach((row,index)=>{if(y<50){pageNumber++;page=pdf.addPage([PAGE.width,PAGE.height]);head();}page.drawRectangle({x:35,y:y-18,width:525,height:21,color:index%2?soft:rgb(1,1,1)});page.drawText(text(row.productCode),{x:42,y:y-10,size:7,font:bold,color:ink});page.drawText(text(row.productName).slice(0,62),{x:125,y:y-10,size:7,font:regular,color:ink});page.drawText(String(row.physicalBalance),{x:430,y:y-10,size:7,font:regular,color:ink});page.drawText(String(row.reservedBalance),{x:482,y:y-10,size:7,font:regular,color:ink});page.drawText(String(row.availableBalance),{x:535,y:y-10,size:7,font:bold,color:row.availableBalance<=0?rgb(.75,.1,.1):ink});y-=21;});
  const bytes=await pdf.save();return new File([new Blob([bytes as BlobPart],{type:"application/pdf"})],`saldo-briland-${new Date().toISOString().slice(0,10)}.pdf`,{type:"application/pdf"});
}
