alter table public."Produto"
  add column if not exists "imagemOriginal" text,
  add column if not exists "imagemCard" text,
  add column if not exists "imagemDetalhe" text;

comment on column public."Produto"."imagemOriginal" is 'Arquivo original preservado para futuras reprocessagens.';
comment on column public."Produto"."imagemCard" is 'WebP leve e permanente usado nas listas e cards do catálogo.';
comment on column public."Produto"."imagemDetalhe" is 'WebP de alta qualidade e permanente usado na tela do produto.';
