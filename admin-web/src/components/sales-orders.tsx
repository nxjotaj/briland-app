...
import { CommercialOrderDashboard } from "@/components/commercial-order-dashboard";
...
const statusStyle: Record<string, string> = {
  ...
  PARTIALLY_INVOICED: "border-amber-200 bg-amber-100 text-amber-900",
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <button onClick={() => setStatus("APPROVED")} className="rounded-[22px] border border-emerald-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"><CheckCircle2 size={19} /></div><div className="mt-5 text-3xl font-black text-slate-950">{orders.filter((o) => o.status === "APPROVED").length}</div><div className="mt-1 text-sm font-black text-slate-900">Aprovados</div><div className="mt-1 text-xs font-semibold text-slate-600">Aguardando faturamento</div></button>
        <button onClick={() => setStatus("INVOICED")} className="rounded-[22px] border border-cyan-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-100 text-cyan-700"><PackageCheck size={19} /></div><div className="mt-5 text-3xl font-black text-slate-950">{orders.filter((o) => o.status === "INVOICED").length}</div><div className="mt-1 text-sm font-black text-slate-900">Faturados</div><div className="mt-1 text-xs font-semibold text-slate-600">NF-e conciliada e estoque baixado</div></button>
      </div>
