...
export function VideoStudio({ products, notify }: { products: Produto[]; notify: (message: string) => void }) {
  ...
  const submit = async () => {
    ...
    if (!headline.trim()) return notify("Informe o título do vídeo.");
    setSubmitting(true);
    const { error } = await supabase.rpc("create_video_render_job", {
      p_product_id: selectedProduct.id,
      p_template_key: templateKey,
      p_format: format,
      p_duration_seconds: duration,
      p_headline: headline.trim(),
      p_subheadline: subheadline.trim(),
      p_cta: cta.trim()
    });
    setSubmitting(false);
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,.9fr)]">
      ...
      <section className="panel-card p-5 lg:p-6"><div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-black">Prévia do layout</h3><span className="rounded-full bg-soft px-3 py-1 text-xs font-black">{format === "vertical" ? "9:16" : "1:1"}</span></div>
        <div className={`relative mx-auto overflow-hidden rounded-[28px] bg-gradient-to-br from-[#061a38] via-[#0a3971] to-[#1372c4] shadow-2xl ${format === "vertical" ? "aspect-[9/16] max-h-[620px]" : "aspect-square max-w-[560px]"}`}>
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between p-[7%] text-white"><div className="text-[clamp(12px,2vw,18px)] font-black tracking-[.16em]">BRILAND</div><Clapperboard className="text-yellow" /></div>
          <div className="absolute inset-x-[7%] top-[18%] bottom-[34%] flex items-center justify-center overflow-hidden rounded-[24px] bg-white/95 p-[6%]">{imageUrl ? <img src={imageUrl} alt={selectedProduct?.nome || "Produto"} className="h-full w-full object-contain" /> : <div className="text-center text-sm font-black text-slate-400">Produto sem imagem</div>}</div>
          <div className="absolute inset-x-[7%] bottom-[7%] text-white"><div className="line-clamp-2 text-[clamp(20px,4vw,42px)] font-black leading-tight">{headline || "Título do produto"}</div><div className="mt-2 line-clamp-2 text-[clamp(11px,1.5vw,17px)] font-semibold text-white/70">{subheadline}</div>{templateKey === "commercial-offer" && <div className="mt-3 text-[clamp(18px,3vw,34px)] font-black text-yellow">{price}</div>}<div className="mt-4 inline-flex rounded-full bg-yellow px-4 py-2 text-xs font-black text-navy">{cta || "Saiba mais"}</div></div>
