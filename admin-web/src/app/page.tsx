...
function friendlyAdminError(error: unknown, action = "concluir esta operação") {
  ...
  const safeDetail = raw.replace(/https?:\/\/\S+/g, "").replace(/\s+/g, " ").trim();
  return safeDetail ? `Não foi possível ${action}. Motivo informado pelo sistema: ${safeDetail}` : `Não foi possível ${action}. O sistema não informou o motivo; tente novamente e, se continuar, informe ao responsável.`;
}
