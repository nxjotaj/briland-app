...
function friendlyAdminError(error: unknown, action = "concluir esta operação") {
  ...
  const parts = errorRecord
    ? [errorRecord.message, errorRecord.details, errorRecord.hint, errorRecord.error_description]
        .filter((value): value is string => typeof value === "string" && Boolean(value.trim()))
    : [];
  const code = errorRecord && typeof errorRecord.code === "string" ? errorRecord.code : "";
  ...
  if (/updatedAt.*Categoria|Categoria.*updatedAt|schema cache.*updatedAt/i.test(raw)) return "A categoria não foi salva porque o formulário tentou enviar uma informação que não existe nesse cadastro. Atualize a página e tente novamente.";
  if (/null value.*nome|nome.*not-null|nome.*required/i.test(raw)) return "Preencha o nome antes de salvar.";
  if (/null value.*slug|slug.*not-null|slug.*required/i.test(raw)) return "Preencha o endereço interno (slug) antes de salvar.";
  if (/selecione a categoria vinculada/i.test(raw)) return "Selecione a categoria vinculada antes de salvar a subcategoria.";
  if (/selecione a subcategoria vinculada/i.test(raw)) return "Selecione a subcategoria vinculada antes de salvar o grupo de produtos.";
  if (code === "42501" || /permission|policy|row-level|rls|not allowed|forbidden|unauthorized/i.test(raw)) return "O sistema recusou o salvamento por falta de autorização da sessão. Ação: atualize a página, entre novamente com uma conta administrativa e tente salvar. Se persistir, a permissão do cadastro precisa ser corrigida no banco.";
  ...
  const safeDetail = raw.replace(/https?:\/\/\S+/g, "[endereço do arquivo]").replace(/\s+/g, " ").trim();
  const codeDetail = code ? ` Código do erro: ${code}.` : "";
  return safeDetail ? `Não foi possível ${action}. Motivo: ${safeDetail}.${codeDetail} Ação: revise os campos informados, atualize a página e tente novamente. Se persistir, envie esta mensagem completa ao responsável.` : `Não foi possível ${action}. O serviço não informou o motivo. Ação: atualize a página e tente novamente; se persistir, o responsável deve consultar o registro técnico da operação.`;
}
...
function CategoryHierarchySection({ data, query, reload, notify, canDelete }: { data: AppData; query: string; reload: () => Promise<void>; notify: (message: string) => void; canDelete: boolean }) {
  ...
  return <>
    ...
    {editing && <TaxonomyModal editing={editing} data={data} reload={reload} notify={notify} canDelete={canDelete} onClose={() => setEditing(null)} />}
  </>;
}

function TaxonomyModal({ editing, data, reload, notify, canDelete, onClose }: { editing: TaxonomyEdit; data: AppData; reload: () => Promise<void>; notify: (message: string) => void; canDelete: boolean; onClose: () => void }) {
  ...
  const save = async () => { setSaving(true); try { const name = draft.nome.trim(); if (!name) throw new Error("O nome é obrigatório."); if (editing.table === "Subcategoria" && !(draft as Subcategoria).categoriaId) throw new Error("Selecione a categoria vinculada."); if (editing.table === "GrupoProduto" && !(draft as GrupoProduto).subcategoriaId) throw new Error("Selecione a subcategoria vinculada."); if (draft.imagem != null && typeof draft.imagem !== "string") throw new Error("A imagem foi enviada, mas o endereço retornou em formato inválido. Selecione a imagem novamente antes de salvar."); const payload: Record<string, unknown> = { nome: name, slug: automaticSlugPreview, descricao: draft.descricao?.trim() || null, imagem: draft.imagem || null, ordem: Number(draft.ordem || 0), ativo: draft.ativo !== false }; if (editing.table !== "Categoria") payload.updatedAt = new Date().toISOString(); if (editing.table === "Subcategoria") payload.categoriaId = (draft as Subcategoria).categoriaId; if (editing.table === "GrupoProduto") payload.subcategoriaId = (draft as GrupoProduto).subcategoriaId; const request = isNew ? supabase.from(editing.table).insert({ id: draft.id, ...payload }) : supabase.from(editing.table).update(payload).eq("id", draft.id); const { data: saved, error } = await request.select("id").maybeSingle(); if (error) throw error; if (!saved) throw new Error("Nenhum cadastro foi atualizado. A sessão perdeu a autorização para editar esta classificação. Atualize a página e entre novamente antes de tentar."); notify(editing.table === "Categoria" ? "Categoria atualizada. Todos os produtos vinculados já exibem o novo nome." : "Classificação salva."); await reload(); onClose(); } catch (error) { notify(friendlyAdminError(error, "salvar a classificação")); } finally { setSaving(false); } };
